...
Tests mirror source: gdw_data_core/core/clients/gcs_client.py

Run tests in isolation to avoid module caching issues:
    pytest gdw_data_core/tests/unit/core/clients/test_gcs_client.py -v
"""
...
def _reload_gcs_client():
    ...
    for mod in list(sys.modules.keys()):
        if 'gcs_client' in mod:
            del sys.modules[mod]
    from gdw_data_core.core.clients.gcs_client import GCSClient
    return GCSClient
...
class TestGCSClientInit:
    ...
    def test_init_success(self):
        ...
        with patch('gdw_data_core.core.clients.gcs_client.storage.Client') as mock_client_class:
