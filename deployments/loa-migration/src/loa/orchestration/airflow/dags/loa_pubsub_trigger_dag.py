"""
LOA Pub/Sub Trigger DAG.

Listens for .ok file arrival via Pub/Sub and triggers the ODP load pipeline.
This is the entry point for the LOA data processing flow.

LOA Pattern: 1 entity (applications) → immediate FDP trigger (no dependency wait)

Flow:
1. Pub/Sub sensor waits for file notification
2. Validates the .ok file and corresponding data file using HDRTRLParser
3. Triggers loa_odp_load_dag

Tags: loa, trigger, pubsub
"""
...
SYSTEM_ID = "LOA"
ENTITIES = ["applications"]  # LOA has only 1 entity
...
PUBSUB_SUBSCRIPTION = Variable.get("loa_pubsub_subscription", default_var="loa-file-notifications-sub")
LANDING_BUCKET = Variable.get("loa_landing_bucket", default_var=f"{PROJECT_ID}-loa-dev-landing")
ERROR_BUCKET = Variable.get("loa_error_bucket", default_var=f"{PROJECT_ID}-loa-dev-error")
...
with DAG(
    dag_id="loa_pubsub_trigger_dag",
    default_args=default_args,
    description="Listen for LOA file arrivals via Pub/Sub and trigger ODP load",
    ...
    def parse_pubsub_message(**context) -> Dict[str, Any]:
        ...
        # LOA only has applications entity
        entity = "applications"

        # Extract date from filename (e.g., loa_applications_20260104.ok)
        extract_date = None
    ...
    trigger_odp = TriggerDagRunOperator(
        ...
        trigger_dag_id="loa_odp_load_dag",
