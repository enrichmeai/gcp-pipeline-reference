...
PUBSUB_SUBSCRIPTION = Variable.get("generic_pubsub_subscription", default_var="generic-file-notifications-sub")
LANDING_BUCKET = Variable.get("generic_landing_bucket", default_var=f"{PROJECT_ID}-generic-dev-landing")
ERROR_BUCKET = Variable.get("generic_error_bucket", default_var=f"{PROJECT_ID}-generic-dev-error")
...
with DAG(
    dag_id="pubsub_trigger_dag",
    default_args=default_args,
    description="Listen for Generic file arrivals via Pub/Sub and trigger ODP load",
    ...
    def parse_pubsub_message(**context) -> Dict[str, Any]:
        ...
        # Extract date from filename (e.g., generic_customers_20260104.ok)
        extract_date = None
