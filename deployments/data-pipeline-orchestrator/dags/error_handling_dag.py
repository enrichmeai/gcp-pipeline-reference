...
Purpose: Monitor and reprocess failed records from Generic pipelines.
...
Tags: generic, error, reprocessing
"""
import os
import json
import logging
...
dag = DAG(
    ...
    catchup=False,
    tags=['generic', 'error', 'reprocessing'],
    description='Monitor and reprocess failed records from Generic pipelines'
)
...
def send_alert_for_critical_errors(**context) -> str:
    ...
    try:
        ...
        if errors:
            message = f"""
            CRITICAL ERRORS DETECTED in Generic Pipelines
...
with dag:
    ...
    with TaskGroup('request_manual_review') as manual_review_group:
        ...
        notify_team = EmailOperator(
            ...
            subject='Generic Pipeline: Manual Error Review Required',
