...
class LOAFileValidator:
    """
    Validates Application2 file structure.
    ...
    Example:
        ...
        >>> result = validator.validate(file_lines, "applications")
        >>> if result.is_valid:
        ...     print(f"Valid file with {result.record_count} records")
    """

    SYSTEM_ID = SYSTEM_ID

    def __init__(self):
        """Initialize with library HDRTRLParser."""
    ...
    def validate(
        ...
        """
        Validate a Application2 entity file.
        ...
        # Step 3: Validate header content (Application2-specific)
        if metadata.header.system_id != self.SYSTEM_ID:
            errors.append(
                f"System ID mismatch: expected {self.SYSTEM_ID}, "
        ...
        data_start = metadata.data_start_line
        data_end = metadata.data_end_line
        data_lines = file_lines[data_start:data_end + 1]

        is_valid, msg = validate_checksum(data_lines, metadata.trailer.checksum)
        if not is_valid:
            errors.append(f"Checksum validation failed: {msg}")

        # Record count excludes CSV header
