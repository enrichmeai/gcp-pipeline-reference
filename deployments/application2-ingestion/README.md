...
**Unit 1 of Application2 3-Unit Deployment**
...
| Directory | Purpose |
|-----------|---------|
| `application2_ingestion/pipeline/` | Beam pipeline and transforms |
| `application2_ingestion/config/` | System configuration |
| `application2_ingestion/schema/` | **Python Schemas**: Source-of-truth `EntitySchema` definitions used for validation and PII masking logic. |
| `application2_ingestion/schemas/` | **JSON Schemas**: Physical BigQuery table schemas used for manual loads, recovery, and external tool integration. |
| `application2_ingestion/validation/` | File and record validators |
...
The Application2 ingestion pipeline demonstrates the **Global Portability** of the library framework. Even with a simple 1:1 mapping, it leverages:
...
2.  **Fluent Pipeline**: Use `BeamPipelineBuilder` to build your pipeline in `src/application2_ingestion/pipeline/`.
...
For detailed infrastructure definitions, see [infrastructure/terraform/systems/application2/ingestion/](../../infrastructure/terraform/systems/application2/ingestion/).
...
```bash
./scripts/setup_deployment_venv.sh application2-ingestion
source deployments/application2-ingestion/venv/bin/activate
```
...
```bash
python -m application2_ingestion.pipeline.main \
...
3.  The `application2_pubsub_trigger_dag` will detect the file and launch a Dataflow job using this ingestion code.
