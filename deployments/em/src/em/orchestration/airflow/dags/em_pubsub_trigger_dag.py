...
PUBSUB_SUBSCRIPTION = Variable.get("em_pubsub_subscription", default_var="em-file-notifications-sub")
LANDING_BUCKET = Variable.get("em_landing_bucket", default_var=f"{PROJECT_ID}-em-dev-landing")
ERROR_BUCKET = Variable.get("em_error_bucket", default_var=f"{PROJECT_ID}-em-dev-error")
...
with DAG(
    dag_id="em_pubsub_trigger_dag",
    default_args=default_args,
    description="Listen for EM file arrivals via Pub/Sub and trigger ODP load",
    ...
    def parse_pubsub_message(**context) -> Dict[str, Any]:
        ...
        # Extract date from filename (e.g., em_customers_20260104.ok)
        extract_date = None
    ...
    trigger_odp = TriggerDagRunOperator(
        ...
        trigger_dag_id="em_odp_load_dag",
