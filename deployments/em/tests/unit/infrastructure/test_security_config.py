...
def get_project_root() -> Path:
    """Get the project root directory (where infrastructure/terraform is located)."""
    # Navigate up from deployments/em/tests/unit/infrastructure/
    current = Path(__file__).resolve()
    # Go up to find the root (contains infrastructure/terraform/)
    for parent in current.parents:
        if (parent / "infrastructure" / "terraform").exists():
            return parent
...
class TestKMSConfiguration:
    ...
    def test_kms_key_ring_exists(self):
        ...
        security_tf_path = PROJECT_ROOT / "infrastructure/terraform/security.tf"
        if not security_tf_path.exists():
            pytest.skip(f"Terraform file not found: {security_tf_path}")

        content = security_tf_path.read_text()
...
class TestEMInfrastructure:
    ...
    def test_em_pubsub_topic_exists(self):
        ...
        em_tf_path = PROJECT_ROOT / "infrastructure/terraform/em/main.tf"
        if not em_tf_path.exists():
            pytest.skip(f"Terraform file not found: {em_tf_path}")

        content = em_tf_path.read_text()
...
class TestSecurityBestPractices:
    ...
    def test_uniform_bucket_access(self):
        ...
        for tf_name in ["infrastructure/terraform/em/main.tf", "infrastructure/terraform/loa/main.tf"]:
            ...
            assert "uniform_bucket_level_access = true" in content, \
                f"Uniform bucket access not enabled in {tf_name}"
    ...
    def test_service_accounts_created(self):
        ...
        for tf_name in ["infrastructure/terraform/em/main.tf", "infrastructure/terraform/loa/main.tf"]:
            ...
            assert "google_service_account" in content, \
                f"Service account not defined in {tf_name}"
    ...
    def test_least_privilege_iam(self):
        ...
        for tf_name in ["infrastructure/terraform/em/main.tf", "infrastructure/terraform/loa/main.tf"]:
            ...
            assert "roles/storage.objectViewer" in content or "roles/storage.objectAdmin" in content, \
                f"Storage IAM roles not properly configured in {tf_name}"
