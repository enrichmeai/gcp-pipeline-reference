"""
Application1 Pub/Sub Trigger DAG.

Listens for .ok file arrival via Pub/Sub and triggers the ODP load pipeline.
This is the entry point for the Application1 data processing flow.

Flow:
1. Pub/Sub sensor waits for file notification
2. Validates the .ok file and corresponding data file using HDRTRLParser
3. Triggers application1_odp_load_dag for the appropriate entity

Tags: application1, trigger, pubsub
"""
...
PUBSUB_SUBSCRIPTION = Variable.get("application1_pubsub_subscription", default_var="application1-file-notifications-sub")
LANDING_BUCKET = Variable.get("application1_landing_bucket", default_var=f"{PROJECT_ID}-application1-dev-landing")
ERROR_BUCKET = Variable.get("application1_error_bucket", default_var=f"{PROJECT_ID}-application1-dev-error")
...
with DAG(
    dag_id="application1_pubsub_trigger_dag",
    default_args=default_args,
    description="Listen for Application1 file arrivals via Pub/Sub and trigger ODP load",
    ...
    def parse_pubsub_message(**context) -> Dict[str, Any]:
        ...
        # Extract date from filename (e.g., application1_customers_20260104.ok)
        extract_date = None
    ...
    trigger_odp = TriggerDagRunOperator(
        ...
        trigger_dag_id="application1_odp_load_dag",
