"""
Application1 ODP Load DAG.

Loads Application1 entity data (customers, accounts, decision) to ODP (BigQuery).
Triggered by application1_pubsub_trigger_dag after file validation.
...
Tags: application1, odp, dataflow
"""
...
DATAFLOW_TEMPLATE_BUCKET = Variable.get("dataflow_templates_bucket", default_var=f"{PROJECT_ID}-application1-dev-temp")
...
def create_job_record(**context):
    ...
    run_id = context.get("run_id", f"application1_{entity}_{extract_date}")
...
def check_all_entities_loaded(**context) -> str:
    """
    Check if all 3 Application1 entities are loaded for the extract date.
...
with DAG(
    dag_id="application1_odp_load_dag",
    default_args=default_args,
    description='Load Application1 entity data to ODP (BigQuery)',
    schedule_interval=None,  # Triggered by application1_pubsub_trigger_dag
    catchup=False,
    tags=['application1', 'odp', 'dataflow'],
) as dag:
    ...
    run_dataflow = BaseDataflowOperator(
        ...
        pipeline_name='application1-odp-load',
        project_id=PROJECT_ID,
        region=REGION,
        ...
        template_path=f'gs://{DATAFLOW_TEMPLATE_BUCKET}/templates/application1_pipeline.json',
        use_template=True, # Set to False to run application1_pipeline.py directly from GCS
        additional_params={
    ...
    trigger_fdp = TriggerDagRunOperator(
        ...
        trigger_dag_id='application1_fdp_transform_dag',
        conf={
            'extract_date': '{{ dag_run.conf.file_metadata.extract_date }}',
            'triggered_by': 'application1_odp_load_dag',
        },
