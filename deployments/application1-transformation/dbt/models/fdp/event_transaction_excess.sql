{{
  config(
    ...
    tags=['fdp', 'application1', 'event']
  )
}}

/*
  Application1 Event Transaction Excess - Foundation Data Product
...
with customers as (
    select * from {{ ref('stg_application1_customers') }}
),

accounts as (
    select * from {{ ref('stg_application1_accounts') }}
),
