# Application2 Transformation

**Unit 2 of Application2 3-Unit Deployment**
...
```
                         Application2 TRANSFORMATION FLOW
                         ───────────────────────
...
| Directory | Purpose |
|-----------|---------|
| `dbt/models/staging/application2/` | Staging models (clean raw data) |
...
The Application2 transformation unit showcases how the `gcp-pipeline-transform` library simplifies system-agnostic transformation:

1.  **Macro Reusability**: Uses `add_audit_columns` and `mask_pii` exactly like Application1, proving the library's "Zero-Bleed" and "Generic-First" implementation.
...
For detailed infrastructure definitions, see [infrastructure/terraform/systems/application2/transformation/](../../infrastructure/terraform/systems/application2/transformation/).
...
```bash
./scripts/setup_deployment_venv.sh application2-transformation
source deployments/application2-transformation/venv/bin/activate
```
...
### 5. Cloud Execution
In production, this unit is triggered by the `application2_odp_load_dag` once ingestion is successful. The transformation is executed via a `BashOperator` running `dbt run`.
...
FROM {{ ref('stg_application2_applications') }}
```
