"""Unit tests for deployments.application1.pipeline.application1_pipeline module."""
...
from application1_ingestion.pipeline.application1_pipeline import (
    AddAuditColumnsDoFn,
    EM_ENTITY_CONFIG,
)
from application1_ingestion.schema import EMCustomerSchema, EMAccountSchema, EMDecisionSchema
...
class TestEMEntityConfig:
    """Tests for Application1 entity configuration."""
...
class TestSchemaValidation:
    ...
    def test_valid_customer_passes(self, application1_customer_record):
        """Valid customer record should pass validation."""
        validator = SchemaValidator(EMCustomerSchema)
        errors = validator.validate(application1_customer_record)
        assert len(errors) == 0

    def test_missing_customer_id_fails(self, application1_customer_record):
        """Missing customer_id should fail validation."""
        application1_customer_record['customer_id'] = ''
        validator = SchemaValidator(EMCustomerSchema)
        errors = validator.validate(application1_customer_record)
        assert len(errors) > 0
        assert any('customer_id' in str(e) for e in errors)

    def test_invalid_status_fails(self, application1_customer_record):
        """Invalid status should fail validation."""
        application1_customer_record['status'] = 'X'
        validator = SchemaValidator(EMCustomerSchema)
        errors = validator.validate(application1_customer_record)
        assert len(errors) > 0
        assert any('status' in str(e) for e in errors)

    def test_valid_account_passes(self, application1_account_record):
        """Valid account record should pass validation."""
        validator = SchemaValidator(EMAccountSchema)
        errors = validator.validate(application1_account_record)
        assert len(errors) == 0

    def test_valid_decision_passes(self, application1_decision_record):
        """Valid decision record should pass validation."""
        validator = SchemaValidator(EMDecisionSchema)
        errors = validator.validate(application1_decision_record)
        assert len(errors) == 0
...
class TestAddAuditColumnsDoFn:
    ...
    def test_adds_run_id(self, application1_customer_record):
        ...
        results = list(dofn.process(application1_customer_record))

        assert len(results) == 1
        assert results[0]['_run_id'] == 'test_run_123'

    def test_adds_source_file(self, application1_customer_record):
    ...
    def test_adds_extract_date(self, application1_customer_record):
    ...
    def test_adds_processed_at(self, application1_customer_record):
