...
**Unit 1 of Application1 3-Unit Deployment**
...
| Directory | Purpose |
|-----------|---------|
| `application1_ingestion/pipeline/` | Beam pipeline and transforms |
| `application1_ingestion/config/` | System configuration |
| `application1_ingestion/schema/` | **Python Schemas**: Source-of-truth `EntitySchema` definitions used for validation and PII masking logic. |
| `application1_ingestion/schemas/` | **JSON Schemas**: Physical BigQuery table schemas used for manual loads, recovery, and external tool integration. |
| `application1_ingestion/validation/` | File and record validators |
...
The Application1 ingestion pipeline is a **Lean Consumer** of the library framework. It achieves complex mainframe ingestion with minimal custom code by leveraging:

1.  **Metadata-Driven Schema**: `application1_ingestion/schema/customers.py` simply defines an `EntitySchema`. The library's `SchemaValidator` handles all type checking and PII masking automatically.
...
For detailed infrastructure definitions, see [infrastructure/terraform/systems/application1/ingestion/](../../infrastructure/terraform/systems/application1/ingestion/).
...
|----------|-------------|----------|
| `--entity` | Application1 entity to process (`customers`, `accounts`, `decision`) | Yes |
...
3.  The `application1_pubsub_trigger_dag` will detect the file and launch a Dataflow job using this ingestion code.
