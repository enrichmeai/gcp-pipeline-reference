"""
Application1 Pipeline Runner.

Main entry point for Application1 Dataflow pipeline.
"""
...
def run_pipeline(argv=None):
    """Run the Application1 pipeline."""

    parser = argparse.ArgumentParser()
    known_args, pipeline_args = parser.parse_known_args(argv)

    pipeline_options = PipelineOptions(pipeline_args)
    application1_options = pipeline_options.view_as(EMPipelineOptions)

    entity = application1_options.entity
    input_file = application1_options.input_file
    output_table = application1_options.output_table
    error_table = application1_options.error_table
    run_id = application1_options.run_id
    extract_date = application1_options.extract_date
    ...
    logger.info(f"Starting Application1 pipeline for {entity}")
    ...
    logger.info(f"Application1 pipeline completed for {entity}")
