"""
Application1 Pipeline Router - Dynamic File Type Routing

Routes incoming files to appropriate pipeline based on file type and content.
Handles Application1 entity types: Customers, Accounts, Decision.

Application1 Flow: 3 entities → ODP tables → Transformation → 2 FDP tables
...
class FileType(Enum):
    """Supported Application1 file types for routing."""
    CUSTOMERS = "customers"
    ACCOUNTS = "accounts"
    DECISION = "decision"
    UNKNOWN = "unknown"
...
class PipelineRouter(BasePipelineRouter):
    """Routes Application1 files to appropriate pipeline."""

    def __init__(self):
        """Initialize router with Application1 pipelines."""
        super().__init__()
        self._register_default_pipelines()

    def _register_default_pipelines(self):
        """Register Application1 pipeline configurations."""
    ...
    def detect_file_type(self, file_path: str) -> FileType:
        """
        Detect Application1 file type from file path and name.
        """
        file_path_lower = file_path.lower()

        # Check for Application1 entity type keywords in path
