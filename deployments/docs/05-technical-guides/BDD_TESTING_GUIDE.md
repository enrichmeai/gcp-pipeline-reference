...
This guide explains how to implement Behavior-Driven Development (BDD) tests in the LOA Blueprint using the standardized `gdw_data_core.testing.bdd` library.

## 🎯 Architecture

We follow a tiered approach to BDD to maximize reusability:

1.  **Core Library (`gdw_data_core`)**: Contains reusable step definitions and base test classes.
...
Before writing new Python code, check if the steps are already available in `gdw_data_core.testing.bdd.steps`:
...
- `When the LOA migration pipeline is triggered for "<job>"`
