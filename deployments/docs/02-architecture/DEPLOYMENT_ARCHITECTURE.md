...
### 🔑 CMEK Infrastructure (PLAT-INF-001)

```
┌──────────────────────────────────────────────────────────────┐
│                    ENCRYPTION LAYER (KMS)                    │
├──────────────────────────────────────────────────────────────┤
│ Key Ring: ${prefix}-keyring (us-central1)                    │
│ ├─ messaging-key     (Pub/Sub encryption, 90-day rotation)   │
│ ├─ storage-key       (GCS encryption, 90-day rotation)       │
│ └─ warehouse-key     (BigQuery encryption, 90-day rotation)  │
└──────────────────────────────────────────────────────────────┘
                              │
          ┌───────────────────┼───────────────────┐
          ▼                   ▼                   ▼
┌─────────────────┐ ┌─────────────────┐ ┌─────────────────┐
│   GCS BUCKETS   │ │  PUB/SUB TOPICS │ │ BIGQUERY DATASETS│
│  (CMEK bound)   │ │  (CMEK bound)   │ │  (CMEK bound)   │
├─────────────────┤ ├─────────────────┤ ├─────────────────┤
│ input-bucket    │ │ notifications   │ │ raw_dataset     │
│ archive-bucket  │ │ audit-events    │ │ staging_dataset │
│ error-bucket    │ │ dead-letter     │ │ marts_dataset   │
└─────────────────┘ └─────────────────┘ └─────────────────┘

Service Agent IAM Bindings:
├─ GCS Agent      → roles/pubsub.publisher (on notification topics)
├─ Pub/Sub Agent  → roles/cloudkms.cryptoKeyEncrypterDecrypter
└─ BigQuery Agent → roles/cloudkms.cryptoKeyEncrypterDecrypter
```
