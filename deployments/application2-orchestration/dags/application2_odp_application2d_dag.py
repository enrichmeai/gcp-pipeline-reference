"""
Application2 ODP Load DAG.

Loads Application2 applications data to ODP (BigQuery).
Triggered by application2_pubsub_trigger_dag after file validation.

Application2 Pattern: 1 entity → immediate FDP trigger (no dependency wait like Application1)
...
Tags: application2, odp, dataflow
"""
...
ENTITY = "applications"  # Application2 has only 1 entity
...
DATAFLOW_TEMPLATE_BUCKET = Variable.get("dataflow_templates_bucket", default_var=f"{PROJECT_ID}-application2-dev-temp")
...
def create_job_record(**context):
    ...
    run_id = context.get("run_id", f"application2_{entity}_{extract_date}")
...
with DAG(
    dag_id="application2_odp_load_dag",
    default_args=default_args,
    description='Load Application2 applications data to ODP (BigQuery)',
    schedule_interval=None,  # Triggered by application2_pubsub_trigger_dag
    catchup=False,
    tags=['application2', 'odp', 'dataflow'],
) as dag:
    ...
    run_dataflow = BaseDataflowOperator(
        ...
        pipeline_name='application2-odp-load',
        ...
        template_path=f'gs://{DATAFLOW_TEMPLATE_BUCKET}/templates/application2_pipeline.json',
        use_template=True, # Set to False to run application2_pipeline.py directly from GCS
        additional_params={
    ...
    # Task 4: Trigger FDP transformation immediately (Application2 has no dependency wait)
    trigger_fdp = TriggerDagRunOperator(
        task_id='trigger_fdp_transform',
        trigger_dag_id='application2_fdp_transform_dag',
        conf={
            'extract_date': '{{ dag_run.conf.file_metadata.extract_date }}',
            'run_id': '{{ ti.xcom_pull(key="run_id") }}',
            'triggered_by': 'application2_odp_load_dag',
        },
    ...
    # Task flow - Application2 immediately triggers FDP (no dependency check like Application1)
