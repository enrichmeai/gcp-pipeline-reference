"""
Application2 FDP Transform DAG.

Runs dbt transformation to map applications into FDP table:
- fdp_loa.portfolio_account_facility

Application2 Pattern: 1 source → 1 target (MAP)

Flow:
1. Run dbt staging model (stg_application2_applications)
2. Run dbt FDP model (MAP to target)
3. Update job control status

Tags: application2, fdp, dbt, transformation
"""
...
# FDP tables - Application2 maps 1 source into 1 target
FDP_TABLES = [
...
def log_transform_start(**context):
    ...
    logger.info(f"Starting Application2 FDP transformation for run_id={run_id}, extract_date={extract_date}")
...
with DAG(
    dag_id='application2_fdp_transform_dag',
    default_args=default_args,
    description='Transform Application2 ODP to FDP (MAP 1 source to 1 table)',
    ...
    staging = BashOperator(
        ...
        bash_command=f'''
            cd {DBT_PROJECT_PATH} && \
            dbt run --select staging.application2 --vars '{{"extract_date": "{{{{ dag_run.conf.extract_date }}}}"}}' --target prod
        ''',
    )
