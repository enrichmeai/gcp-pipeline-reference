...
- [GCP_DEPLOYMENT_CONFIGURATION.md](GCP_DEPLOYMENT_CONFIGURATION.md) — GCP project setup and API enablement
...
| # | Deployment | Runtime | Docker Image | Terraform Module |
|---|-----------|---------|--------------|------------------|
| 1 | `original-data-to-bigqueryload` | Cloud Dataflow (Flex Template) | `generic-ingestion` | `ingestion/` |
| 2 | `bigquery-to-mapped-product` | BigQuery (dbt) | `generic-transformation` | `transformation/` |
| 3 | `data-pipeline-orchestrator` | Cloud Composer (Airflow) | `generic-dag-validator` | `orchestration/` |
...
|----------|-------------|------------|
| **GCS Bucket** (landing) | `{PROJECT_ID}-generic-{ENV}-landing` | Terraform `ingestion/` |
| **GCS Bucket** (archive) | `{PROJECT_ID}-generic-{ENV}-archive` | Terraform `ingestion/` |
| **GCS Bucket** (error) | `{PROJECT_ID}-generic-{ENV}-error` | Terraform `ingestion/` |
| **GCS Bucket** (temp/templates) | `{PROJECT_ID}-generic-{ENV}-temp` | Terraform `ingestion/` |
| **BigQuery Dataset** | `odp_generic` | Terraform `ingestion/` |
| **BigQuery Tables** | `customers`, `accounts`, `decision`, `applications` | Terraform `ingestion/` |
| **BigQuery Error Tables** | `customers_errors`, `accounts_errors`, `decision_errors`, `applications_errors` | Terraform `ingestion/` |
| **BigQuery Dataset** | `job_control` | Terraform `ingestion/` |
| **BigQuery Tables** | `pipeline_jobs`, `audit_trail` | Terraform `ingestion/` |
| **Pub/Sub Topic** | `generic-file-notifications` | Terraform `ingestion/` |
| **Pub/Sub Subscription** | `generic-file-notifications-sub` | Terraform `ingestion/` |
| **Pub/Sub Dead Letter** | `generic-file-notifications-dead-letter` | Terraform `ingestion/` |
...
|------|--------------|-------------------------------|
| GCS bucket names | Terraform `ingestion/variables.tf` | `gcp_project_id` + `environment` variables |
| BigQuery dataset location | Terraform `ingestion/variables.tf` | `bq_location` (default: `EU`) |
| Bucket lifecycle rules | Terraform `ingestion/main.tf` | Landing: 90d→COLDLINE, Archive: 365d→COLDLINE→5y→ARCHIVE, Error: 90d→DELETE |
| Force destroy flag | Terraform `ingestion/variables.tf` | `force_destroy` — `true` for dev/int, `false` for prod |
| Versioning | Terraform `ingestion/variables.tf` | `enable_versioning` — `true` recommended for all envs |
...
| Resource | Name Pattern | Created By |
|----------|-------------|------------|
| **BigQuery Dataset** | `fdp_generic` | Terraform `transformation/` |
...
docker run --rm \
  ...
  gcr.io/${PROJECT_ID}/generic-transformation:1.0.10 \
...
| Resource | Name Pattern | Created By |
|----------|-------------|------------|
| **Cloud Composer** | `generic-{ENV}-composer` | Terraform `orchestration/` |
| **Service Account** | `generic-composer-sa@{PROJECT_ID}.iam.gserviceaccount.com` | Terraform `orchestration/` |
...
| `generic_landing_bucket` | `{PROJECT_ID}-generic-dev-landing` | pubsub_trigger_dag |
| `generic_error_bucket` | `{PROJECT_ID}-generic-dev-error` | error_handling_dag |
| `dataflow_templates_bucket` | `{PROJECT_ID}-generic-dev-temp` | data_ingestion_dag |
...
| Item | Where Defined | What to Change Per Environment |
|------|--------------|-------------------------------|
| Composer environment name | Terraform `orchestration/main.tf` | `generic-{ENV}-composer` |
| Composer image version | Terraform `orchestration/main.tf` | `composer-2.10.2-airflow-2.10.5` |
| Composer size/scaling | Terraform `orchestration/main.tf` | Worker min/max, CPU, memory |
| Airflow env variables | Terraform `orchestration/main.tf` | Bucket names, dataset names, project ID |
...
| **Docker** | Image tag | `1.0.10` | Pinned release version |
...
# Install framework
pip install gcp-pipeline-framework>=1.0.10
