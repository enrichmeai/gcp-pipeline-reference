...
This guide provides a standardized breakdown of major tasks required to onboard a new legacy system migration using the framework. Use these tasks to create Jira tickets or project plans.

---

## 📋 Task Hierarchy

A standard system migration is broken down into **4 Major Work Streams**, corresponding to our 3-Unit Deployment model plus initial analysis.
...
2.  **Define Entity Schema**: Create `EntitySchema` definitions in `gcp-pipeline-core` (or system-specific config) including data types, nullability, and regex patterns.
...
1.  **Initialize Ingestion Project**: Create `deployments/<system_id>-ingestion/` structure.
...
1.  **Initialize dbt Project**: Create `deployments/<system_id>-transformation/dbt/`.
...
1.  **Initialize Orchestration Project**: Create `deployments/<system_id>-orchestration/dags/`.
