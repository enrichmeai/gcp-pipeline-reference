...
Instead of `em-ingestion` and `loa-ingestion`, we deploy a single **Universal Ingestion Unit**.
...
2.  **Dynamic Selectors**: Airflow triggers dbt using specific selectors:
    ```bash
    dbt run --select tag:system_{{ system_id }} --vars '{...}'
    ```
...
2.  **YAML Definition Example**:
    ...
    entities:
      - name: "customers"
        type: "MAP"
        ingestion_config: "gs://config/em/customers.json"
    ```
