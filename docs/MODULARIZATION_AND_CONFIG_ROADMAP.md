...
This roadmap defines how to transition the framework from systapplication1-specific code to a **Configuration-First Deployment Model**. By assuming the libraries are separated, we can treat Ingestion and Transformation as "Generic Engines" that are controlled by external manifests.
...
Instead of `application1-ingestion` and `application2-ingestion`, we deploy a single **Universal Ingestion Unit**.
...
2.  **Runtime Parameters**: When Airflow triggers the job, it passes:
    ...
    *   `--systapplication1_id`: Used for logging and metadata.
...
2.  **Dynamic Selectors**: Airflow triggers dbt using specific selectors:
    ```bash
    dbt run --select tag:systapplication1_{{ systapplication1_id }} --vars '{...}'
    ```
...
2.  **YAML Definition Example**:
    ...
    entities:
      - name: "customers"
        type: "MAP"
        ingestion_config: "gs://config/application1/customers.json"
    ```
