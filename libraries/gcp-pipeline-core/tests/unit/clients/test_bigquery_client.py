...
def _reload_bigquery_client():
    ...
    for mod in list(sys.modules.keys()):
        if 'bigquery_client' in mod or 'gcp_pipeline_core.clients' == mod:
            del sys.modules[mod]
    from gcp_pipeline_core.clients.bigquery_client import BigQueryClient
...
class TestBigQueryClientInit:
    ...
    def test_initialization(self):
        ...
        with patch('gcp_pipeline_core.clients.bigquery_client.bigquery.Client'):
...
class TestBigQueryClientWriteTable:
    ...
    def test_write_to_table_success(self):
        ...
        with patch('gcp_pipeline_core.clients.bigquery_client.bigquery.Client') as mock_bq:
            BigQueryClient = _reload_bigquery_client()
            client = BigQueryClient(project='test-project', dataset='test_dataset')
