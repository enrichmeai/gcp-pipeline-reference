...
resource "google_bigquery_table" "pipeline_jobs" {
  ...
  schema = jsonencode([
    ...
    { name = "system_id", type = "STRING", mode = "REQUIRED", description = "System (application1, application2)" },
...
# Dataflow service account for Application1 pipelines
resource "google_service_account" "application1_dataflow" {
  account_id   = "${local.prefix}-dataflow"
  display_name = "Application1 Dataflow Service Account"
  description  = "Service account for Application1 Dataflow pipeline execution"
}

# dbt service account for Application1 transformations
resource "google_service_account" "application1_dbt" {
  account_id   = "${local.prefix}-dbt"
  display_name = "Application1 dbt Service Account"
  description  = "Service account for Application1 dbt transformations"
}
...
resource "google_project_iam_member" "application1_dataflow_worker" {
  project = var.gcp_project_id
  role    = "roles/dataflow.worker"
  member  = "serviceAccount:${google_service_account.application1_dataflow.email}"
}

# Dataflow - GCS permissions
resource "google_storage_bucket_iam_member" "application1_dataflow_landing" {
  bucket = google_storage_bucket.landing.name
  role   = "roles/storage.objectAdmin"
  member = "serviceAccount:${google_service_account.application1_dataflow.email}"
}

resource "google_storage_bucket_iam_member" "application1_dataflow_archive" {
  bucket = google_storage_bucket.archive.name
  role   = "roles/storage.objectAdmin"
  member = "serviceAccount:${google_service_account.application1_dataflow.email}"
}

resource "google_storage_bucket_iam_member" "application1_dataflow_error" {
...
resource "google_bigquery_dataset_iam_member" "application1_dataflow_odp" {
...
resource "google_bigquery_dataset_iam_member" "application1_dataflow_job_control" {
  dataset_id = google_bigquery_dataset.job_control.dataset_id
  role       = "roles/bigquery.dataEditor"
  member     = "serviceAccount:${google_service_account.application1_dataflow.email}"
}
...
resource "google_pubsub_subscription_iam_member" "application1_dataflow_subscriber" {
  subscription = google_pubsub_subscription.application1_file_notifications_sub.name
  role         = "roles/pubsub.subscriber"
  member       = "serviceAccount:${google_service_account.application1_dataflow.email}"
}

# dbt - BigQuery permissions
resource "google_bigquery_dataset_iam_member" "application1_dbt_odp_reader" {
  dataset_id = google_bigquery_dataset.odp_application1.dataset_id
  role       = "roles/bigquery.dataViewer"
  member     = "serviceAccount:${google_service_account.application1_dbt.email}"
}

resource "google_bigquery_dataset_iam_member" "application1_dbt_fdp_editor" {
  dataset_id = google_bigquery_dataset.fdp_application1.dataset_id
  role       = "roles/bigquery.dataEditor"
  member     = "serviceAccount:${google_service_account.application1_dbt.email}"
}

resource "google_bigquery_dataset_iam_member" "application1_dbt_job_control" {
  dataset_id = google_bigquery_dataset.job_control.dataset_id
  role       = "roles/bigquery.dataViewer"
  member     = "serviceAccount:${google_service_account.application1_dbt.email}"
}
...
# Cloud Composer Environment for Application1 orchestration
resource "google_composer_environment" "application1_composer" {
  ...
  config {
    software_config {
      ...
      env_variables = {
        ...
        ODP_DATASET       = google_bigquery_dataset.odp_application1.dataset_id
        FDP_DATASET       = google_bigquery_dataset.fdp_application1.dataset_id
    ...
    node_config {
      service_account = google_service_account.application1_composer.email
    }
  }
  ...
  depends_on = [
    google_project_iam_member.application1_composer_worker,
  ]
}

# Service account for Cloud Composer
resource "google_service_account" "application1_composer" {
  account_id   = "application1-composer-sa"
  display_name = "Application1 Cloud Composer Service Account"
}

# Composer service account IAM roles
resource "google_project_iam_member" "application1_composer_worker" {
  project = var.gcp_project_id
  role    = "roles/composer.worker"
  member  = "serviceAccount:${google_service_account.application1_composer.email}"
}

resource "google_project_iam_member" "application1_composer_dataflow" {
  project = var.gcp_project_id
  role    = "roles/dataflow.admin"
  member  = "serviceAccount:${google_service_account.application1_composer.email}"
}

resource "google_project_iam_member" "application1_composer_bigquery" {
  project = var.gcp_project_id
  role    = "roles/bigquery.admin"
  member  = "serviceAccount:${google_service_account.application1_composer.email}"
}

resource "google_project_iam_member" "application1_composer_storage" {
  project = var.gcp_project_id
  role    = "roles/storage.admin"
  member  = "serviceAccount:${google_service_account.application1_composer.email}"
}

resource "google_pubsub_subscription_iam_member" "application1_composer_subscriber" {
  subscription = google_pubsub_subscription.application1_file_notifications_sub.name
  role         = "roles/pubsub.subscriber"
  member       = "serviceAccount:${google_service_account.application1_composer.email}"
}
